...
ubyte[] packLength(size_t l, out uint offset)
{
   ubyte[] t;
   if (!l)
   {
...
protected:
   ...
   uint buildAuthPacket()
   {
      ...
      *p++ = 33;
      ...
      foreach (size_t i; 0.._user.length)
         *p++ = _user[i];
      *p++ = 0;
